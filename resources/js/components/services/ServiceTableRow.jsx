...
const ServiceTableRow = ({
  ...
  const hasQuestions =
    service.has_active_questions && service.active_question?.questions_json?.length > 0;
  ...
  const FewOptionsChips = ({ options }) => (
    <div className="flex flex-wrap gap-2">
      {options.map((o, i) => (
        <Badge key={i} variant="outline" className="text-xs px-2 py-1 flex flex-col items-start">
          <span className="mr-1 font-medium">{o.name}</span>
          <span className="text-muted-foreground">{formatPrice(o.price)}</span>
        </Badge>
      ))}
    </div>
  );
        ...
        <Table>
          ...
          <TableBody>
            {options.map((option, index) => (
              <TableRow key={index}>
                <TableCell className="text-sm py-2">{option.name}</TableCell>
      ...
      {isExpanded && (
        <TableRow key={`${service.id}-details`}>
          <TableCell colSpan={8} className="p-0">
            <div className="p-3">
              {/* Header row inside details: left title + right "View Images" */}
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="bg-white rounded-md border shadow-sm p-3 md:col-span-1">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {service.description ? (
                      <span className="whitespace-pre-wrap break-words">{service.description}</span>
                    ) : (
                  ...
                  <div className="bg-white rounded-md border shadow-sm p-3 md:col-span-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-sm font-semibold">
                        {isMultiSelect ? "Available Options (Multi-Select)" : "Available Options"}
                      </h3>
                      {isMultiSelect && (
                        <Badge
                          variant="outline"
                    ...
                    {fewOptions && <FewOptionsChips options={service.options} />}
                    {manyOptions && <ManyOptionsTable options={service.options} />}
                  </div>
                )}
                ...
                {hasQuestions && (
                  <div className="bg-white rounded-md border shadow-sm p-3 md:col-span-1">
                    ...
                    {hasQuestionsFew && <QuestionsInline questions={questions} />}
                    {hasQuestionsMany && <QuestionsScrollable questions={questions} />}
                  </div>
                )}
              </div>
